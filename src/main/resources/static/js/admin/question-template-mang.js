...
//    alert( jQuery(this).parents("div").eq(1).attr('class') );
    fnTabClick_QuestionMang({topClass:'frame-left'});
  });
  jQuery('.frame-right > .tab_wrap > .tab > li').click(function() {
    PD_Common.fnTabActive(this);
    fnTabClick_QuestionMang({topClass:'frame-right'});
  });
}
 ...
 * @param arrParams
 */
function fnTabClick_QuestionMang(arrParams) {
  ...
  switch (type) {
    case "list":
      PD_Common.fnOpenContent("/admin/question-template/question-template-list", "tab_contents_edge_left", params);
      break;
    case "form1":
    case "form2":
      if(params) {
      ...
      PD_Common.fnOpenContent("/admin/question-template/question-template-" + type, "tab_contents_edge_right", params);
      break;
    default:
      break;
  }
